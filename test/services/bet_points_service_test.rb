...
class BetPointsServiceTest < ActiveSupport::TestCase
  test '.call! focus on Bet' do
    perfect_bet = bets(:diego_game_1) # B 4:3 | A 4:3
    draw_bet = bets(:diego_game_2) # B 0:0 | A 1:1
    home_wins_bet = bets(:diego_game_13) # B 1:0 | A 2:0
    guest_wins_bet = bets(:diego_game_14) # B 0:1 | A 0:2
    failed_bet = bets(:diego_game_3) # B 0:2 | A 2:1
    wrong_bet = bets(:diego_game_4) # B 1:0 | A 1:2
    ...
    assert_equal 6, perfect_bet.reload.total_points
    assert_equal 1, perfect_bet.home_team_score_points
    assert_equal 1, perfect_bet.guest_team_score_points
    assert_equal 3, perfect_bet.result_points
    assert_equal 1, perfect_bet.perfect_bet_bonus_points
    ...
    assert_equal 4, home_wins_bet.reload.total_points
    assert_equal 0, home_wins_bet.home_team_score_points
    assert_equal 1, home_wins_bet.guest_team_score_points
    assert_equal 3, home_wins_bet.result_points
    assert_equal 0, home_wins_bet.perfect_bet_bonus_points

    assert_equal 4, guest_wins_bet.reload.total_points
    assert_equal 1, guest_wins_bet.home_team_score_points
  ...
  test '.call! focus on User.total_points' do
    ...
    assert_changes -> { user.reload.total_points }, to: 126 do
      BetPointsService.new.call!
    end
  end
